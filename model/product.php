...
class product
{
    ...
    //Lấy thông tin danh mục theo ID
    // ==================================================
    function getPageById($id, $start, $limit)
    {
        $db = new connect();
        $select = "SELECT * FROM product LIMIT $start,$limit";
        $product_array = $db->getList($select);
        return $product_array;
    }
    ...
    {
        $db = new connect();
        $select = "select * from product where id_category=$id LIMIT 8";
        $result = $db->getList($select);
        return $result;
    }
    // ==================================================
    ...
    // Đếm SL SP theo ID CATE
    function countProductListPage($id)
    {
        $db = new connect();
        $select = "select count(*) as total from product where id_category=$id";
        $result = $db->getInstance($select);
        return $result;
    }
    // ==================================================
    //Đếm số lượng product by category
    // ==================================================
    function count_prd_cate($id)
    {
        $db = new connect();
        $select = "select count(*) as total_cate from product where id_category=$id";
        $result = $db->getInstance($select);
        return $result;
    }
    // Tìm kiếm sản phẩm
    ...
    // ==================================================
    function getProductByPage($entries, $id_category, $page)
    {
        if ($page == 1) {
            $tmpPage = $page - 1;
        } else {
        ...
        $db = new connect();
        $select = "select * from product where id_category=$id_category order by id_product limit $tmpPage, $entries";
        $result = $db->getList($select);
        return $result;
    }
}